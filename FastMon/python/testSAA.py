...
if __name__ == '__main__':
    ...
    canvas.Divide(2, 2)
    canvas.cd(1); gLon.Draw('ap')
    canvas.cd(2); gLat.Draw('ap')
    canvas.cd(3); gSAADist.Draw('ap'); gSAAFlag.Draw('p,same')
    canvas.cd(4); EARTH_GRID.Draw();
    gOrb.Draw('p,same'); parser.SAAPolygon.draw('same')
    canvas.Update()
